...
def parse_args():
    ...
    parser.add_argument('--dataset-pickle', type=str, default="/home/1739678401/2024.9dataclean/MMGNN_0901/data/exp_data.csv")
    ...
    # output dir
    parser.add_argument('--save-dir', type=str, default="/home/1739678401/2024.9dataclean/MMGNN_0901/result/exp")
...
def train(optimizer, model, nodes,nodes1, data_loader, loss_fn, metric_fn, device=None):
    ...
    for it, (solute_batched_graph, solvent_batched_graph, label) in enumerate(data_loader):
        ...
        #print(f"Memory allocated before batch: {torch.cuda.memory_allocated(device) / 1024**2:.2f} MB")
        ...
        #print(solute_feats)
        #print(f"Memory allocated before model: {torch.cuda.memory_allocated(device) / 1024**2:.2f} MB")
        ...
        #print(f"Memory allocated after model: {torch.cuda.memory_allocated(device) / 1024**2:.2f} MB")
        pred = pred.view(-1)
...
def main_worker(gpu, world_size, args):
    ...
    print(f"Memory allocated before training: {torch.cuda.memory_allocated(args.gpu) / 1024**2:.2f} MB")
    # 开始训练前记录时间
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if not args.distributed or (args.distributed and args.gpu == 0):
            ...
            print(f"Memory allocated after epoch {epoch}: {torch.cuda.memory_allocated(args.gpu) / 1024**2:.2f} MB")
            ls.append( val_acc)
            if epoch % 10 == 0:
                sys.stdout.flush()
    ...
    print(f"Memory allocated after training: {torch.cuda.memory_allocated(args.gpu) / 1024**2:.2f} MB")
